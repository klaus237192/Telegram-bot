...
def sendMessage(data):  # sourcery skip: extract-method
    ...
    if config.send_discord_alerts:
        try:
            webhook = DiscordWebhook(
                url=f"https://discord.com/api/webhooks/{config.discord_webhook}"
            )
            embed = DiscordEmbed(title=data)
    ...
    if config.send_slack_alerts:
        try:
            ...
            slack = Slack(
                url=f"https://hooks.slack.com/services/{config.slack_webhook}"
            )
            slack.post(text=data)
        except Exception as e:
            print("[X] Slack Error:\n>", e)
    ...
    if config.send_twitter_alerts:
        tw_auth = OAuthHandler(config.tw_ckey, config.tw_csecret)
        tw_auth.set_access_token(config.tw_atoken, config.tw_asecret)
        tw_api = API(tw_auth)
        try:
            tw_api.update_status(
                status=data.replace("*", "").replace("_", "").replace("`", "")
            )
        except Exception as e:
